...
- (TGShowMoreVC *)showMoreVC {
    ...
    if (_showMoreVC.collectionView.superview != (_parentV ? _parentV : self.superview)) {
        _showMoreVC.collectionView.frame = CGRectMake(0,  ([self.superview isKindOfClass:UINavigationBar.class] ? CGRectGetMinY(self.superview.frame) : 0) + maxY , [UIScreen mainScreen].bounds.size.width, 0);
...
- (UIView *)coverV {
    if (!_coverV) {
        CGFloat maxY = CGRectGetMaxY(self.frame);
        _coverV = [[UIView alloc] initWithFrame:CGRectMake(0,  ([self.superview isKindOfClass:UINavigationBar.class] ? CGRectGetMinY(self.superview.frame) : 0) + maxY, [UIScreen mainScreen].bounds.size.width, 0)];
...
//        UIImage *hideImage = [UIImage imageWithContentsOfFile:hideImgPath];
        ...
        UIView *lineV = [[UIView alloc] initWithFrame:CGRectMake(self.segmentConfig.limitMargin * 0.5, (self.segmentConfig.segmentBarH - self.segmentConfig.showMoreBtnlineViewH) * 0.5 - self.segmentConfig.indicatorH * 2, 1, self.segmentConfig.showMoreBtnlineViewH)];
...
- (UIView *)indicatorV {
    if (!_indicatorV) {
        _indicatorV = [[UIView alloc] initWithFrame:CGRectMake(0, self.height - self.segmentConfig.indicatorH, 0, self.segmentConfig.indicatorH)];
...
-(void)setSegmentModels:(NSArray<id<TGSegmentModelProtocol>> *)segmentModels{
    ...
    for (NSObject<TGSegmentModelProtocol> *segM in segmentModels) {
        UIButton *btn = [[UIButton alloc] init];
        btn.tag = segM.segID == -1 ? self.segBtns.count : segM.segID;
...
- (void)segmentClick: (UIButton *)btn {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        self.indicatorV.y = self.contentScrollV.height - self.segmentConfig.indicatorH;
...
//- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
//    CGPoint p = [self convertPoint:point toView:self.showMoreVC.collectionView];
//    if ([self.showMoreVC.collectionView pointInside:p withEvent:event]) {
//        return self.showMoreVC.collectionView;
//    }
//    return [super hitTest:point withEvent:event];
//}
//
//- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
//    CGPoint p =[self.showMoreVC.collectionView convertPoint:point fromView:self];
//    if ([self.showMoreVC.collectionView pointInside:p withEvent:event]) {
//        return NO;
//    }
